...
function sendverificationEmail(n) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(n) {
            n.Error == null ? (t ? $(".profile_verify_button.email").remove() : document.getElementsByClassName("tbl-list")[1].style.display = "none",
            n.Message && $("#js_email_verify_msg").html(n.Message).removeClass("error_color").addClass("succsess_color")) : n.Message && $("#js_email_verify_msg").html(n.Message).removeClass("succsess_color").addClass("error_color")
        },
        error: function(n) {
...
function sendverificationMobile(n) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(n) {
            n.Error == null ? (t ? $(".profile_verify_button.mobile").remove() : $("#sendverificationMobile").closest(".tbl-list").css("display", "none"),
            n.Message && $("#js_mobile_verify_msg").html(n.Message).removeClass("error_color").addClass("succsess_color")) : n.Message && $("#js_mobile_verify_msg").html(n.Message).removeClass("succsess_color").addClass("error_color")
        },
        error: function(n) {
...
function resendCodeAfter30Sec(n) {
    var t;
    if (clearInterval(interval30),
    n == "email") {
        event.stopPropagation();
        $("#verificationEmail").attr("disabled", "disabled");
        t = 30;
        $("#verificationEmail").attr("disabled", "disabled");
        interval30 = setInterval(i, 1e3);
        function i() {
            t == 0 ? (clearInterval(interval30),
            $("#verificationEmail").removeAttr("disabled")) : (t--,
            $("#verificationEmail").find($(".profile_verify__timer")).html(t))
        }
    } else if (n == "mobile") {
        event.stopPropagation();
        ...
        function i() {
            t == 0 ? (clearInterval(interval30),
            $("#verificationMobile").removeAttr("disabled")) : (t--,
            $("#verificationMobile").find($(".profile_verify__timer")).html(t))
        }
    }
}
...
function ResponseHandling(n, t) {
    var i = "#" + t;
    n.Success === !0 ? ($(i).find(".errormassage").html(""),
    $(i).find(".successmassage").html(n.Message),
    $(i).find("input:enabled").val("")) : n.Code == "148" ? $(".trigger").click() : n.Code == "418" ? ($(".trigger").click(),
    $("#CancelActiveBonusYes").attr("disabled", "disabled"),
    $("#CancelActiveBonusYes").css("display", "none"),
    $("#CancelActiveBonusNo").text("Ok"),
    $("#ActiveBonusNotCancalble").text("You Can not make the Withdrawal until your bonus will be expired or is finished")) : ($(i).find(".successmassage").html(""),
    $(i).find(".errormassage").html(n.Message))
}
function LoginTrigger() {
    $(".platformMobLog").attr("href").toLocaleLowerCase().includes("/login") ? document.location.href = "/Login/Login" : $(".platformMobLog").trigger("click")
}
function responsibleGamingChecks(n) {
    var t = n.ShowRealityCheckPopup;
    t == "yes" && getRealityCheckPopupInfo()
}
var JsonPost = {}, interval30, BalanceUpdater;
...
$.fn.serializeObject = function() {
    ...
    return $.each(t, function() {
        n[this.name] !== undefined ? (n[this.name].push || (n[this.name] = [n[this.name]]),
        ...
        $("tr").on("click", function(t) {
            ...
            !$(this).hasClass("message")) {
                ...
                $.ajax({
                    ...
                    success: function(n) {
                        if (r.find(".message-content").html(n.Message),
                        f.hasClass("unread")) {
...
BalanceUpdater = {
    ...
    _isRunning: !1,
    start: function() {
        this._isRunning || (this._intervalId = setInterval(this.getBalanceAmount.bind(this), 6e4),
        this._isRunning = !0)
    },
    tryUpdateStatus: function() {
        var n = this;
        this._isRunning || (n._intervalId && clearTimeout(n._intervalId),
        $.ajax({
            ...
            success: function(t) {
                if ((typeof regClicked == "undefined" || !window.regClicked) && t && t.Reload == "Reload") {
                    if (document.location.href.toLowerCase().indexOf("registration/register2") > -1)
                        return;
                    document.location.reload()
                } else
                    n._intervalId = setTimeout(n.tryUpdateStatus.bind(n), 6e4)
            }
        }))
    },
    stop: function() {
    ...
    getBalanceAmount: function() {
        var i = this, n, t;
        this.PlayerId > 0 && (n = !1,
        location.pathname.toLowerCase().includes("sport") && !location.pathname.toLowerCase().includes("virtualsports") && (n = !0),
        t = $.param({
            isSportUrl: n
        }),
        $.ajax({
    ...
    onBalanceStatus: function(n) {
        n.Balance ? (window.playerBalance && (window.playerBalance.innerText = n.Balance),
        window.playerBalance1 && (window.playerBalance1.innerText = n.Balance),
        window.playerBalance2 && (window.playerBalance2.innerText = n.Balance1),
        window.playerBonusBalance && (isNaN(n.Bonus1) ? (window.bonusBalanceCont.classList.remove("hidden"),
        window.playerBonusBalance.innerHTML = n.Bonus1) : n.Bonus1 != 0 ? (window.playerBonusBalance.innerHTML = n.Bonus1.toFixed(2),
        window.bonusBalanceCont.classList.remove("hidden")) : window.bonusBalanceCont.classList.add("hidden")),
        n.CommonBalance && (window.mobCommonBalance && (window.mobCommonBalance.innerText = n.CommonBalance),
        window.mobCommonBalanceAccount && (window.mobCommonBalanceAccount.innerText = n.CommonBalance)),
        ...
        window.playerNonWagerable1 && (window.playerNonWagerable1.innerText = n.NonWagerable1)) : n.LogOut == "1" && document.location.reload()
    }
}
